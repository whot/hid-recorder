            ...
            match hut {
                Ok(hut) => {
                    ...
                    match u {
                        ...
                        Err(_) => {
                            if size < 16 {
                                format!("Usage_i{size} (0x{:02x})", u16::from(usage_id))
                            } else {
                                format!("Usage_i{size} (0x{:04x})", u16::from(usage_id))
                            }
                        }
                    }
                }
                Err(_) => format!("Usage_i{size} ({usage_id})"),
            }
        }
